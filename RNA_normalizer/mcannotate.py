...
class MCAnnotate:
    ...
    def _init_dir(self, pdb_file, tmp_dir="tmp"):
        """
        Initialise the temporary directory
        :param pdb_file: .pdb path
        :param tmp_dir: the directory to store the .mcout files
        :return:
        """
        os.makedirs(tmp_dir, exist_ok=True)
        basename = os.path.basename(pdb_file)
        file = os.path.join(tmp_dir, basename + ".mcout")
        return file
    ...
    def parse(self):
        ...
        pattern_stack = (
            "^([A-Z]|'[0-9]'|)(\d+)-([A-Z]|'[0-9]'|)(\d+) :.*(inward|upward|downward|outward).*"
        )
...
if __name__ == "__main__":
    ...
    pdb_file = "example/custom_example_1.pdb"
    out_file = "example/"
    mca.load(pdb_file)
