    ...
    for res_data in res_list:
        ...
        if res_id < 0:
            ...
            if len(sub_residues) < count:
                print(
                    "!!! Error: less than '%d' residues in chain '%s' starting from nt '%d'"
                    % (count, chain, res_id)
                )
                quit()
...
Examples:
$ python fit.py AAAA.pdb BBBB.pdb A:1:10,A:21:5,B:2:9 X:1:10 result.pdb\b\b"""
            % ("- " * 40, "- " * 40)
        )
    ...
    print(
        "RMSD: %s vs %s = %f"
        % (os.path.basename(sys.argv[1]), os.path.basename(sys.argv[2]), rmsd)
    )
